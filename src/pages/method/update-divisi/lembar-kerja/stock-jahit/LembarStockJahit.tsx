...
export default function LembarStockJahit() {
    ...
    const handleConfirm = async () => {
        ...
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Stock Jahit selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/stock-jahit/antrian'), 600);
        }
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Stock Jahit {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
