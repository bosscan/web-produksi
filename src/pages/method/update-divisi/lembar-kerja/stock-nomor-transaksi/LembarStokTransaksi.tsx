...
export default function LembarStockTransaksi() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_stock_no_transaksi', 'selesaiStockNt');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Stock Nomor Transaksi selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/stock-nomor-transaksi/antrian'), 600);
        }
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Stock Nomor Transaksi {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
