...
export default function LembarStockBordir() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_stock_bordir', 'selesaiStockBordir');
        setOpen(false);
    setSnack({ open: true, message: updated ? 'Stock Bordir selesai. Kembali ke antrian.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
    if (updated) setTimeout(() => navigate('/method/update-divisi/stock-bordir/antrian'), 600);
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Stock Bordir {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
