...
export default function LembarFotoProduk() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_foto_produk', 'selesaiFotoProduk');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Foto Produk selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/foto-produk/antrian'), 600);
        }
    };
