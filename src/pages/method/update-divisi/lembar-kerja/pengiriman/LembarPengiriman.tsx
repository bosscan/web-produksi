...
export default function LembarPengiriman() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_pengiriman', 'selesaiPengiriman');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Pengiriman selesai.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/pengiriman/antrian'), 600);
        }
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Pengiriman {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
