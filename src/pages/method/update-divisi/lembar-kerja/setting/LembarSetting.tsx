...
export default function LembarSetting() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_setting', 'selesaiSetting');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Setting selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            // Redirect kembali ke antrian Setting setelah update berhasil
            setTimeout(() => navigate('/method/update-divisi/setting/antrian'), 600);
        }
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Setting {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
