...
export default function LembarFinishing() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_finishing', 'selesaiFinishing');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Finishing selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/finishing/antrian'), 600);
        }
    };
