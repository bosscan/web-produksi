...
export default function LembarJahit() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_jahit', 'selesaiJahit');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Jahit selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/jahit/antrian'), 600);
        }
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Jahit {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
