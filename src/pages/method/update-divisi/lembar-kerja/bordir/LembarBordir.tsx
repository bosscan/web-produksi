...
export default function LembarBordir() {
    ...
    const handleConfirm = async () => {
        const updated = await markSelesai(spkId, 'selesai_bordir', 'selesaiBordir');
        setOpen(false);
        setSnack({ open: true, message: updated ? 'Bordir selesai. Masuk ke antrian berikutnya.' : 'ID SPK tidak ditemukan.', severity: updated ? 'success' : 'info' });
        if (updated) {
            setTimeout(() => navigate('/method/update-divisi/bordir/antrian'), 600);
        }
    };
        ...
        <Box sx={{
            ...
            <Box sx={{
                ...
                <Typography variant="h6" fontWeight={700} align="center" sx={{ width: '100%' }}>
                    Lembar Kerja Divisi Bordir {spkId ? `(SPK: ${spkId})` : ''}
                </Typography>
