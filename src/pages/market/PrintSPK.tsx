...
function mergeSpkData(idSpk: string) {
  ...
  const designNext = antrianPengerjaan.find((d: AnyRec) => String(d?.idSpk ?? '').trim() === String(idSpk).trim());
  // Jika snapshot belum punya mockup, fallback ke design_queue worksheet atau thumbnail map agar tetap tampil
  try {
    ...
    if (!hasSnapshotMock) {
      if (fromQueueMock) {
        ...
        try {
          ...
          if (!qid) {
            qid = (designNext as AnyRec)?.queueId;
          }
          // Jika designNext punya worksheet mockup langsung, gunakan itu terlebih dahulu
          if (!designSnap?.mockupUrl && designNext?.worksheet?.mockup?.file) {
            designSnap = { ...(designSnap || {}), worksheet: designNext.worksheet, mockupUrl: designNext.worksheet.mockup.file };
          }
          const thumb = qid ? map[qid] : undefined;
          if (thumb) {
            designSnap = { ...(designSnap || {}), mockupUrl: thumb };
          }
        } catch {}
      }
    }
    // Pastikan worksheet dari lembar kerja desainer selalu dibawa meskipun tidak ada mockup,
    // agar UKURAN/JARAK di SPK bersumber dari input desainer, bukan otomatis.
    if ((!designSnap || !designSnap.worksheet) && (design?.worksheet || designNext?.worksheet)) {
      designSnap = { ...(designSnap || {}), worksheet: (designNext?.worksheet || design?.worksheet) };
    }
  } catch {}
...
export default function PrintSPK() {
  ...
  // Helper to downscale uploaded images for snapshot
  const downscaleImage = (dataUrl: string, maxSize = 1000, quality = 0.82): Promise<string> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        try { const out = canvas.toDataURL('image/jpeg', quality); resolve(out || dataUrl); } catch { resolve(dataUrl); }
      };
  ...
  <Button variant="outlined" onClick={() => generatePdf(false)} disabled={!data}>Unduh PDF</Button>
        ...
        {/* Utilities to fix legacy items without embedded mockup */}
        <label htmlFor="upload-mockup" style={{ marginLeft: 8 }}>
          <input id="upload-mockup" type="file" accept="image/*" style={{ display: 'none' }}
            onChange={async (e) => {
              ...
              reader.onloadend = async () => {
                ...
                const id = idSpk || inputId;
                if (id) persistDesignSnapshot(id, { mockupUrl: thumb });
              };
              reader.readAsDataURL(file);
            }} />
          <Button component="span" variant="outlined" size="small" disabled={!data}>Upload Mockup</Button>
        </label>
        <Button variant="outlined" size="small" onClick={() => pullThumbnailToSnapshot()} disabled={!data}>Ambil dari Thumbnail</Button>
      </Stack>
      ...
      <tr><td align="left" style={{ padding: 4, verticalAlign: 'top', textAlign: 'left', whiteSpace: 'pre-wrap', fontSize: 9, lineHeight: 1.2 }}>{data.alamat || ''}</td></tr>
          ...
          <Box sx={{ display: 'flex', gap: '3px', flexGrow: 1, overflow: 'hidden' }}>
            ...
            <Box sx={{ flexBasis: '65%', display: 'flex', flexDirection: 'column', gap: '3px' }}>
              {/* Perbesar mockup 3x dari ukuran terkunci sebelumnya (17.5% -> 52.5%) */}
              <Box sx={{ flex: '0 0 52.5%', border: '2px solid #000', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                {(() => {
              ...
              {/* Atribut fixed height (locked) */}
              <Box sx={{ flex: '0 0 47.5%', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '3px', overflow: 'hidden' }}>
                {(() => {
                  const ws = ((data as any)?.designSnap?.worksheet || {}) as AnyRec;
                  const assets = Array.isArray((data as any)?.designSnap?.assets) ? (data as any).designSnap.assets as AnyRec[] : [];
                  const blocks: Array<{ label: string; b?: AnyRec }> = [
                    { label: 'ATRIBUT LENGAN KANAN', b: ws?.lenganKanan },
                    { label: 'ATRIBUT DADA KANAN', b: ws?.dadaKanan },
                    { label: 'ATRIBUT BELAKANG', b: ws?.belakang },
                    { label: 'ATRIBUT LENGAN KIRI', b: ws?.lenganKiri },
                    { label: 'ATRIBUT DADA KIRI', b: ws?.dadaKiri },
                    { label: 'ATRIBUT TAMBAHAN', b: ws?.tambahanReferensi },
                  ];
                  return blocks.map(({ label, b }) => {
                    const ukuran = (b?.ukuran ?? '').toString();
                    const jarak = (b?.jarak ?? '').toString();
                    const keterangan = (b?.keterangan ?? '').toString();
                    const key = label.toLowerCase();
                    const imgFromWs = b?.file as string | undefined;
                    const imgFromAssets = assets.find((a: AnyRec) => {
                      const attr = String(a?.attribute || '').toLowerCase();
                      // cari kecocokan longgar berdasarkan label
                      if (key.includes('lengan kanan')) return attr.includes('lengan kanan');
                      if (key.includes('lengan kiri')) return attr.includes('lengan kiri');
                      if (key.includes('dada kanan')) return attr.includes('dada kanan');
                      if (key.includes('dada kiri')) return attr.includes('dada kiri');
                      if (key.includes('belakang')) return attr.includes('belakang');
                      if (key.includes('tambahan')) return attr.includes('tambahan') || attr.includes('referensi');
                      return false;
                    })?.file as string | undefined;
                    const imageUrl = imgFromWs || imgFromAssets;
                    return (
                      <Box key={label} sx={{ border: '2px solid #000', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', p: '2px', fontSize: 8 }}>
                        <Box sx={{ fontWeight: 700, fontSize: 7, textAlign: 'left', px: '4px', py: '2px' }}>{label}</Box>
                        <Box sx={{ flexGrow: 1, width: '100%', minHeight: '20px', textAlign: 'left', px: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {imageUrl ? (
                        ...
                        <Box sx={{ flexShrink: 0, width: '60%', px: '4px', pb: '2px' }}>
                          <table style={{ width: '100%', fontWeight: 'normal' }}>
                            <tbody>
                              <tr>
                                <td style={{ width: '45%', fontWeight: 700 }}>UKURAN</td>
                                <td style={{ width: '5%' }}>:</td>
                                <td style={{ width: '50%', fontWeight: 700, wordBreak: 'break-word' }}>{ukuran}</td>
                              </tr>
                              <tr>
                                <td style={{ width: '45%', fontWeight: 700 }}>JARAK</td>
                                <td style={{ width: '5%' }}>:</td>
                                <td style={{ width: '50%', fontWeight: 700, wordBreak: 'break-word' }}>{jarak}</td>
                              </tr>
                              <tr>
                                <td style={{ width: '45%', fontWeight: 700 }}>KETERANGAN</td>
                                <td style={{ width: '5%' }}>:</td>
                                <td style={{ width: '50%', fontWeight: 700, wordBreak: 'break-word' }}>{keterangan}</td>
                              </tr>
                            </tbody>
                          </table>
                        </Box>
...
function formatDate(d: any): string {
  // Format: dd-mm-yyyy HH:mm:ss WIB (Asia/Jakarta)
  if (!d) return '';
  try {
    ...
    const MM = get('minute');
    const SS = get('second');
    if (dd && mm && yyyy && HH && MM && SS) return `${dd}-${mm}-${yyyy} ${HH}:${MM}:${SS} WIB`;
    // Fallback to locale string if parts missing
    return new Intl.DateTimeFormat('id-ID', { timeZone: 'Asia/Jakarta' }).format(date);
